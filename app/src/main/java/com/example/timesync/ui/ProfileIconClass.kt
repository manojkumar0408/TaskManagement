...
class ProfileIconClass (ctx: Context, attrs: AttributeSet) :
    ...
    companion object {
        ...
        fun getRoundedCroppedBitmap(bitmap: Bitmap, radius: Int): Bitmap {
            ...
            if (bitmap.width != radius || bitmap.height != radius) finalBitmap =
                Bitmap.createScaledBitmap(
                    bitmap, radius, radius, false
                )
            else finalBitmap = bitmap
            val output = Bitmap.createBitmap(
                finalBitmap.width, finalBitmap.height, Config.ARGB_8888
            )
            val canvas = Canvas(output)

            val paint = Paint()
            val rect = Rect(
                0, 0, finalBitmap.width, finalBitmap.height
            )
